...
- **YOLO pose model** — Added YOLOv11-pose as default pose model for pre-annotation
...
- **Degenerate crop detection + full annotation health in PDF** — Script 1 now detects degenerate crops (`min(W,H) < 3px`) during bbox extraction, logs them separately from zero-size crops, and writes both to `temp_ann_file.txt`. Script 4 PDF Page 1 dataset overview table now surfaces all annotation health metrics as rows: Bad Crops (empty + tiny <3px, with ⚠ when non-zero), Empty Annotation Files, Invalid Annotation Files, Errors Reading Image Files.
...
- **PCA before DBSCAN (Script 3 + master script)** — Added `--pca_components` arg (default 128) to Script 3. Applies per-class PCA (768d→128d, L2-normalize → PCA → L2-normalize) before DBSCAN and kNN. Fixes curse of dimensionality for large classes (10K+ samples). Exposed as prompt in master script under "Change defaults?" gate with note that this runs before clustering. Set 0 to disable.
